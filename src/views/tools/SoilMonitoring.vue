...
export default {
  ...
  computed: {
    ...
    getPast24Hours() {
      return {
        sensor_id: this.selectId,
        start_time: dayjs().subtract(24, 'hours').format("YYYY-MM-DDTHH:mm:ssZ"),
  ...
  methods: {
      ...
      const option = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            ...
            let result = '';
            const formattedTime = dayjs(params[0].axisValue).format('YYYY年MM月DD日HH:mm:ss');
    ...
    fetchData() {
      ...
      this.$axios.get('/soil-sensor/data', {
          ...
          if (this.data && this.data.length > 0) {
              this.$nextTick(() => {
                  console.log('Checking if chart container is available:', this.$refs.chartContainer);
    ...
    // 右三小表
    init24hCharts(){
      this.temperatureChart = markRaw(echarts.init(this.$refs.temperatureChart));
      this.updateTemperatureChart();

      this.humidityChart = markRaw(echarts.init(this.$refs.humidityChart));
      this.updateHumidityChart();

      this.phChart = markRaw(echarts.init(this.$refs.phChart));
      this.updatePhChart();
    },
    // 更新温度图表
    updateTemperatureChart() {
      ...
      const groupedByHour = this.data.reduce((acc, item) => {
        const hour = dayjs(item.sensor_data.detect_time).format('YYYY-MM-DD HH:00');
        if (!acc[hour]) {
      ...
      const averageTemperatureByHour = Object.keys(groupedByHour).map(hour => ({
        time: hour,
        avgTemp: groupedByHour[hour].sum / groupedByHour[hour].count
      }));

      const maxTemperature = Math.max(...averageTemperatureByHour.map(item => item.avgTemp));
      const minTemperature = Math.min(...averageTemperatureByHour.map(item => item.avgTemp));
      ...
      const temperatureOption = {
        ...
        yAxis: {
          ...
          // 设置间隔
          interval: (Math.ceil(maxTemperature + 1) - Math.floor(minTemperature - 1))
        },
        series: [{
    ...
    updateHumidityChart() {
      ...
      const humidityOption = {
        ...
        series: [{
          ...
          data: [
            { value: latestHumidity, name: '当前湿度', itemStyle: { color: '#80a6e2' } },  // 蓝色
            { value: 100 - latestHumidity, name: '其他', itemStyle: { color: '#a4514f' } }  // 红色
          ]
        }],
        graphic: {
          elements: [
...
updatePhChart() {
  ...
  // 获取最新的数据点
  const latestData = this.data[this.data.length - 1];
  const latestPh = latestData.sensor_data.ph;

  // 计算最小值，最大值和平均值
  const phValues = this.data.map(item => item.sensor_data.ph);
  const minPh = Math.min(...phValues).toFixed(1);
  const maxPh = Math.max(...phValues).toFixed(1);
  const avgPh = (phValues.reduce((a, b) => a + b, 0) / phValues.length).toFixed(1);
  ...
  const phOption = {
    ...
    series: [{
      ...
      detail: { 
        formatter: '{value}',
        offsetCenter: [0, '70%']  // 向下移动当前pH值的位置
      },
      min: 0,
      max: 14,
      data: [{ value: latestPh }],
      axisLine: { // 控制表盘外圈样式
        lineStyle: {
          ...
          color: [[1, new echarts.graphic.LinearGradient(0, 0, 1, 0, [{
            offset: 0, color: '#a4514f' }, { offset: 1, color: '#80a6e2' }])]]
        }
      },
      splitLine: {  //设置分隔线样式
  ...
  this.phChart.setOption(phOption);
},
    fetch24HourData() {
              ...
              <template #overlay>
                <a-menu>
                  <a-menu-item 
                    ...
                    :key="index" 
                    @click="selectId = item.id; fetchData(); fetch24HourData()">
                    <span>
      ...
      <div class="main">
          ...
          <div class="right-panel">
            <div class="info-box">
              ...
              <div ref="temperatureChart" style="height: 110px;width: 100%;"></div>
            </div>
            <div class="info-box">
