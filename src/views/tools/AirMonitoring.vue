  ...
  <div>
    ...
    <a-row style="margin-top: 20px">
      ...
      <a-row style="width: 100%" v-if="!loading">
        <a-descriptions
          :title="`${item.sensor_id}号空气传感器`"
          bordered
          class="data"
          v-for="(item, index) in data"
          :key="index"
        >
          <a-descriptions-item label="传感器编号">{{
            item.sensor_id
          }}</a-descriptions-item>
          <a-descriptions-item label="传感器信息" :span="2">{{
            item.sensor_info
          }}</a-descriptions-item>
          <a-descriptions-item label="二氧化碳浓度">{{
            item.sensor_data.co2concentration
          }}</a-descriptions-item>
