...
updateCO2Chart() {
  ...
  // 计算最小值，最大值和平均值
  const co2Values = this.data.map(item => item.sensor_data.co2concentration);
  const minCO2 = Math.min(...co2Values).toFixed(1);
  const maxCO2 = Math.max(...co2Values).toFixed(1);
  const avgCO2 = (co2Values.reduce((a, b) => a + b, 0) / co2Values.length).toFixed(1);
      ...
      axisLine: { // 控制表盘外圈样式
        lineStyle: {
          ...
          color: [[1, new echarts.graphic.LinearGradient(0, 0, 1, 0, [{
            offset: 0, color: '#ffffff' }, { offset: 1, color: '#80a6e2' }])]]
        }
      },
      splitLine: {  //设置分隔线样式
